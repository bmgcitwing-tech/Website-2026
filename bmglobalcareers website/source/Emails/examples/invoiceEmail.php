...
@ob_start();
include ('config.php');
$qsetting=mysqli_query($conn,"select * from orders ORDER BY ID DESC LIMIT 0, 1");
$setting=mysqli_fetch_array($qsetting);
$cartitems = $setting['pid'];
$servicesitems = $setting['combo'];
$workExp = $setting['workExp'];
                ...
                <tr style="border-collapse:collapse;">
                  <td align="left" style="padding:0;Margin:0;padding-left:35px;padding-right:35px;padding-top:40px;"><table width="100%" cellspacing="0" cellpadding="0" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;">
                      <tr style="border-collapse:collapse;">
						...
						$getcosdswemfworkExp=mysqli_query($conn,"SELECT * FROM category where id IN(".$workExp.") ORDER bY id ASC" );
						$setting12132=mysqli_fetch_array($getcosdswemfworkExp);
                       ...
                       // $getcosdswemf=mysqli_query($conn,"SELECT * FROM category where id IN(".$cartitems.") ORDER bY id ASC" );
						...
						foreach (array_filter($mainItemsArray) as $key => $value) {
						$count++;
						?>
						<tr align="center" class="item">
                                    <td><?php echo $count; ?></td>
                                    <td>998399</td>
                                    <td>COMBO DEAL <br> <?php echo str_replace("CV - ","",$setting12132['name']); ?> <br>CV + <?php echo str_replace("-"," + ",$value); ?></td>
                                    <td><?php echo $mainPriceArray[$key];
  $mainTotal = $mainTotal + $mainPriceArray[$key];
   ?></td>
                                    <td><?php if(!empty($discount_applied)){
										  $discountedPrice1 = $mainPriceArray[$key] * $discount_applied / 100;
										echo number_format((float)$discountedPrice1, 2, '.', '');  // Outputs -> 105.00
									}else{
										echo "N/A";
									}  ?></td>
                                    <td><?php  $discountedpriceAfter = $mainPriceArray[$key] - $discountedPrice1;
                                    ...
                                    <td><?php
									 ...
									 echo round($newprice);
									 $gsttotal = $gsttotal + round($newprice);
									 }
   ...
   ?></td>
									...
									if($newprice < 1)
									 ...
									 echo round($newprice);
									 $gsttotal11 = $gsttotal11 + round($newprice);
									 }
   ...
   ?></td>
									...
									$mainGrandTotal = $mainGrandTotal + $final;
									 ...
									 //$final = round($newprice+$result22['price']);
									echo number_format((float)$final, 2, '.', '');  // Outputs -> 105.00
									$mainGrandTotal = $mainGrandTotal + $final;
									}
   ...
   ?>
                                  <tr align="center" class="item">
                                    ...
                                    <td><?php if(!empty($discountedpriceAfter435435)){
										  $maindisadded =  $discountedPrice1;
									...
									if($active9percent == 1)
										{
									echo number_format((float)$gsttotal, 2, '.', '');  // Outputs -> 105.00
									//echo $gsttotal;
									} ?></td>
                                    <td><?php 
									if($active9percent == 1)
										{
									echo number_format((float)$gsttotal11, 2, '.', '');  // Outputs -> 105.00
									//echo $gsttotal11;
									} ?></td>
                                    <td><?php 
									if($active9percent == 0)
