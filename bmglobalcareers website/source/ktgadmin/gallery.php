			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_gallery WHERE delete_status='NDL' order by gallery_id desc";
						...
						<tr>
                            ...
                            <td><img class="img-responsive" src ="userdata/images/gallery/<?php echo $fetch_sql['imgname']; ?>" width="100px"></td> 
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="gallery_update.php?id=<?php echo $fetch_sql['gallery_id']; ?>" >
									...
									<a href="javascript:void(0)" class="deleteTriger" data-value="<?php echo $fetch_sql['gallery_id']; ?>">
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		e.preventDefault();
		 if($("#photo_name").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_gallery.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/gallery_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
...
$(document).on('click','.statuschanger', function(){
	...
	if (alertmsg == true) {
		$.ajax({
			type: "POST",
			url: "phpservices/gallery_status_changer.php",
			dataType:'json',
