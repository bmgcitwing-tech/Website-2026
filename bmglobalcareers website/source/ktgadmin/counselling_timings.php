...
<main class="main-content">
    ...
    <div class="container-fluid">
        <div class="page-header d-md-flex justify-content-between align-items-center">
            <h4>Counselling centers Timings</h4>
        </div>
    </div>
	<div class="row">
        <div class="col-md-12">
						...
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									<label>Counselling Center :</label>
									<input type="hidden" name="action" value="add" />
									<select class="form-control" name="center_id">
									 <option>Select Counselling Center</option>
                             <?php $chkDoctor=mysqli_query($conn, "SELECT * FROM tbl_counselling_centers");
									while($getDoctor=mysqli_fetch_assoc($chkDoctor)){ ?>
                    <option value="<?php echo $getDoctor['title']; ?>" <?php if($val == $getDoctor['title']){ ?>selected <?php } ?>><img class="img-responsive" src ="userdata/images/banner/<?php echo $getDoctor['imgname']; ?>"> &nbsp;<?php echo $getDoctor['title']; ?></option>
					<?php } ?>
					</select>
								</div>
							</div>
							...
							<div class="col-md-3">
								<div class="form-group">
									<label>Counselling Timing :</label>
							...
							<div class="col-md-3">
											<div class="form-group">
									<label>Counselling Center Timings  : </label>
									<input type="text" class="form-control" id="timiings"  name="timiings">
								</div>
							</div>
			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
                    ...
                    <tr>
                        <th>S.no</th>
                        <th>Counselling Center Name</th>
                        <th>Counselling Center Timings</th>
                        <th>Last&nbsp;Update</th>
                        <th>Action</th>
                    </tr>
                    </thead>
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_counselling_centers_timings WHERE delete_status='NDL' order by ban_id desc";
						...
						<tr>
							<td><?php echo $i; ?></td>
							<td><?php echo str_replace("^","'",$fetch_sql['center_id']); ?></td>
                           <td><?php echo str_replace("^","'",$fetch_sql['duration']); ?> &nbsp;<?php echo str_replace("^","'",$fetch_sql['timiings']); ?></td>
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="counselling_update.php?id=<?php echo $fetch_sql['ban_id']; ?>" >
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		...
		if($("#title").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_counselling_timings.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/counselling_timings_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
