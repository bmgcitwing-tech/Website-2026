						...
						<div class="row">
							<div class="col-md-3">
								<div class="form-group">
									...
									<input type="date" class="form-control" placeholder="Enter Project Name" name="disabled_date" id="disabled_date" autocomplete="off">
								</div>
							</div>
							<div class="col-md-3">
									<div class="form-group">
			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_disabled_dates WHERE delete_status='NDL' order by id desc";
						...
						<tr>
							<td><?php echo $i; ?></td>
							<td><?php echo str_replace("^","'",$fetch_sql['disabled_date']); ?></td>
							<td><?php echo str_replace("^","'",$fetch_sql['description']); ?></td>
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="counselling_appointments_date_update.php?id=<?php echo $fetch_sql['id']; ?>" >
...
$(document).ready(function(){
	...
	$("#submit").click(function(e){
		...
		if($("#disabled_date").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_appointments_date.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/appointments_date_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
