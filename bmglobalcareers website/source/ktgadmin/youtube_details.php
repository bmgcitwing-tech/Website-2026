			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT t2.*,date_format(t2.create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(t2.update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM  tbl_youtube_details t2 WHERE t2.delete_status='NDL' order by vcat_dt_id desc";
						...
						<tr>
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="youtube_update.php?id=<?php echo $fetch_sql['vcat_dt_id']; ?>" >
...
$("#detailsgrid").dataTable({"responsive":true});	
	...
	$("#submit").click(function(e){
		...
		if($("#link").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_youtube.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/youtube_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
