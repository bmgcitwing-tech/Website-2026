						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<textarea class="form-control" id="link" name="link" placeholder="Enter Link " style="min-height:36px;height:32px;"></textarea>
								</div>
							</div>
							<div class="col-md-2">
								<div class="form-group">
			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_social_media WHERE delete_status='NDL' order by social_media_id desc";
						...
						<tr>
							...
							<!-- <td><?php echo str_replace("^","'",$fetch_sql['year']); ?></td> -->
                            <td><img class="img-responsive" src ="userdata/images/social_media/<?php echo $fetch_sql['imgname']; ?>" width="100px"></td> 
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="social_media_update.php?id=<?php echo $fetch_sql['social_media_id']; ?>" >
									...
									<a href="javascript:void(0)" class="deleteTriger" data-value="<?php echo $fetch_sql['social_media_id']; ?>">
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		...
		if($("#title").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_social_media.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/social_media_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
...
$(document).on('click','.statuschanger', function(){
	...
	if (alertmsg == true) {
		$.ajax({
			type: "POST",
			url: "phpservices/social_media_status_changer.php",
			dataType:'json',
