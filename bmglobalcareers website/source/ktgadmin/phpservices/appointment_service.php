...
class appointments_services{
	...
	function appointments_autobind($curdate){
		$bannerData=array();
		$sql= "select *,date_format(person_dob,'%d/%m/%Y') as person_dob,date_format(appoint_date,'%d/%m/%Y') as appoint_date from tbl_counselling_appointments where ban_id='".$_POST['ban_id']."' and delete_status='NDL'";
	    $exec_query=mysqli_query($this->mySQLconn, $sql);
