			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_banner WHERE delete_status='NDL' order by ban_id desc";
						...
						<tr>
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="banner_update.php?id=<?php echo $fetch_sql['ban_id']; ?>" >
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		...
		if($("#title").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_banner.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/banner_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
