			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_clients WHERE delete_status='NDL' order by clients_id desc";
						...
						<tr>
                            ...
                            <td><img class="img-responsive" src ="userdata/images/clients/<?php echo $fetch_sql['imgname']; ?>" width="100px"></td> 
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="clients_update.php?id=<?php echo $fetch_sql['clients_id']; ?>" >
									...
									<a href="javascript:void(0)" class="deleteTriger" data-value="<?php echo $fetch_sql['clients_id']; ?>">
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		...
		// else if($("#year").val()==''){
		// 	$("#msg").css("color","red").html('* Enter Year');
		// 	setTimeout(function(){$("#msg").html('')},3000);
		// 	return false;
		// }
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_clients.php",
					dataType:'json',
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/clients_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
...
$(document).on('click','.statuschanger', function(){
	...
	if (alertmsg == true) {
		$.ajax({
			type: "POST",
			url: "phpservices/clients_status_changer.php",
			dataType:'json',
			data: {statusid:statusid,cond:"clients"},
			success: function(result) {
				if(result.op==1){
