						...
						<div class="row">
							<div class="col-md-3">
	                            <div class="form-group">
	                                <label>Category : <span style="color:red;font-size: 20px;font-weight: bold;">*</span><span><a href="javascript:void(0)" id="exampleModalCenter11" data-toggle="modal"
	                    data-target="#exampleModalCenter"><i class="fa fa-plus-circle" style="font-size:25px;position:absolute;margin-left:6px;color:red;"></i></a></span></label>
	                                <select class="form-control page_cat_id" id="cat_id" name="cat_id">
	                                    <option value="">Select Category</option>
                         <?php    								 
                          $sql = "SELECT t2.*,date_format(t2.create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(t2.update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM  tbl_category t2 WHERE t2.delete_status='NDL' order by id desc";
                          $exec_sql=mysqli_query($conn,$sql);
                          $i=1;
                          while($fetch_sql=mysqli_fetch_array($exec_sql)) {
                        ?>
                                        <option value="<?php echo str_replace("^","'",$fetch_sql['id']); ?>"><?php echo str_replace("^","'",$fetch_sql['title']); ?></option>
                        <?php $i++; } ?>
	                                </select>
	                            </div>
	                        </div>
							...
							<div class="col-md-3">
								<div class="form-group">
									<span id="msg" style="font-weight:bold;margin-top:12px;"></span>
								</div>
							</div>
						</div>
                    </form>
			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT t2.*,date_format(t2.create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(t2.update_date,'%d/%m/%Y %H:%i:%s') as update_date,t1.title AS cattitle  FROM  tbl_videos_details t2 LEFT JOIN tbl_category t1 ON t1.id = t2.cat_id WHERE t2.delete_status='NDL' order by vcat_dt_id desc";
                              $exec_sql=mysqli_query($conn,$sql);
                              $i=1;
                              while($fetch_sql=mysqli_fetch_array($exec_sql)) {
                            ?>
						<tr>
							<td><?php echo $i; ?></td>
							<td><?php echo str_replace("^","'",$fetch_sql['cattitle']); ?></td>
							<td><?php echo str_replace("^","'",$fetch_sql['title']); ?></td>
                            <td><?php echo $fetch_sql['url']; ?></td> 
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="video_update.php?id=<?php echo $fetch_sql['vcat_dt_id']; ?>" >
									...
									<a href="javascript:void(0)" class="deleteTriger" data-value="<?php echo $fetch_sql['vcat_dt_id']; ?>">
										...
										<?php    								 
					                     ...
					                     ?>
											<tr>
												...
												?>
												<td><div class="btn-group" role="group" aria-label="Basic example">
													<a href="category_update.php?id=<?php echo $fetch_sql['id']; ?>" >
														<button type="button" class="btn btn-success" style="padding:3px 5px;margin-right:5px">
															<i class="fa fa-edit" style="color: #fff;padding: 3px;margin: 0;font-size: 20px;"></i>
														</button>
													</a>
														<a href="javascript:void(0)" class="deleteTriger1" data-value="<?php echo $fetch_sql['id']; ?>">
															<button type="button" class="btn btn-danger" style="padding:3px 5px;">
																<i class="fa fa-trash" style="color: #fff;padding: 3px;margin: 0;font-size: 20px;"></i>
															</button>
														</a>
...
$("#detailsgrid").dataTable({"responsive":true});	
	...
	$("#submit").click(function(e){
		...
		if($("#link").val()==''){
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_videos.php",
					dataType:'json',
	...
	$("#cat_submit").click(function(e){
		e.preventDefault();
		if($("#cat_name").val()==''){
			$("#cat_msg").css("color","red").html('* Enter Link');
			...
			$.ajax({
					type: "POST",
					url: "phpservices/add_cat.php",
					dataType:'json',
					data: $("#catform").serialize(),
					success: function(result) {
						console.log(result.op);
						if(result.op==1){
							$("#cat_msg").css("color","green").html("Successfully Saved !");
							setTimeout(function(){
								location.reload();
							},1500);
						}
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/video_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
	...
	$(document).on('click','.deleteTriger1', function(){
		var delid=$(this).data('value');
		var alertmsg=confirm("Do you want to Delete ?");
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/cat_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
