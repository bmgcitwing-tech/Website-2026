			...
			<div class="card">
                <div class="card-body">
                   <table id="detailsgrid" class="table table-striped table-bordered" width="100%" cellspacing="0">
					...
					<tbody>
					<?php    								 
                              $sql = "SELECT *,date_format(create_date,'%d/%m/%Y %H:%i:%s') as create_date,date_format(update_date,'%d/%m/%Y %H:%i:%s') as update_date FROM tbl_team_member WHERE delete_status='NDL' order by id desc";
						...
						<tr>
							...
							<td><?php echo str_replace("^","'",$fetch_sql['designation']); ?></td>
                            <td>
					        <?php if ($fetch_sql['imgname']=='') { ?>
					        <img class="img-responsive" style="width: 100px;" src="https://www.kindpng.com/picc/m/252-2524695_dummy-profile-image-jpg-hd-png-download.png" alt="">
					        <?php } else { ?>
					        <img class="img-responsive" style="width: 100px;" src ="userdata/images/team_member/<?php echo $fetch_sql['imgname']; ?>" width="100px">
					        <?php } ?>
					        </td> 
							<?php 
							...
							<td><div class="btn-group" role="group" aria-label="Basic example">
									<a href="team_update.php?id=<?php echo $fetch_sql['id']; ?>" >
									...
									<a href="javascript:void(0)" class="deleteTriger" data-value="<?php echo $fetch_sql['id']; ?>">
...
$("#photo").change(function(){
	...
	$("#submit").click(function(e){
		...
		if($("#title").val()==''){
			...
			$("#msg").css("color","red").html('* Enter Designation');
			setTimeout(function(){$("#msg").html('')},3000);
			return false;
		}
		else{
	...
	$(document).on('click','.deleteTriger', function(){
		...
		if (alertmsg == true) {
			$.ajax({
						type: "POST",
						url: "phpservices/team_delete.php",
						dataType:'json',
						data: {delid:delid},
						success: function(result) {
							if(result.op==1){
